...
class NormalController extends Controller
{
    ...
    function index(Request $request){
        ...
        $ds_sach_moi = DB::select('SELECT s.*, ten_tac_gia FROM bs_sach s JOIN bs_tac_gia tg ON s.id_tac_gia = tg.id  ORDER BY id DESC LIMIT 8');
        //echo '<pre>',print_r($ds_dsach_noi_bat),'</pre>';

        return view('trang_chu')
        ->with('ds_sach_noi_bat', $ds_sach_noi_bat)
        ->with('ds_sach_moi', $ds_sach_moi);
        //return '123';
    }
